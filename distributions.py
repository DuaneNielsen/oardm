...
import numpy as np
import torch
from torch import nn, distributions as dst
...
def mean(logit_probs, means, coeffs, xs):
    x = means
    x0 = x[:, :, :, 0].clamp(min=-1, max=1.)
    x1 = (x[:, :, :, 1] + coeffs[:, :, :, 0] * x0).clamp(-1., 1.)
    x2 = (x[:, :, :, 2] + coeffs[:, :, :, 1] * x0 + coeffs[:, :, :, 2] * x1).clamp(-1, 1)

    x = torch.stack((x0, x1, x2), dim=-1)

    out = (x * logit_probs.exp().unsqueeze(-1)).sum(dim=-2)
...
def discretized_mix_logistic_loss(x, l):
    ...
    logit_probs = torch.log_softmax(logit_probs, dim=-1)
    ...
    log_probs = torch.sum(log_probs, dim=3) + logit_probs
    return torch.logsumexp(log_probs, dim=-1), mean(logit_probs, means, coeffs, xs)
