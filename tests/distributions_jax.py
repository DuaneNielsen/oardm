...
class SoftmaxCategorical(flax.linen.Module):
    ...
    def log_prob(self, x, dist_params):
        ...
        x_onehot = distribution_utils_jax.onehot(x, num_classes=self.n_classes)
    ...
    def sample(self, key, params):
        ...
        sample_x = distribution_utils_jax.sample_categorical(key, logits)
        return sample_x
    ...
    def get_probs_coding(self, params, selection, x=None):
        ...
        probs = distribution_utils_jax.get_safe_probs(logits)
...
class DiscretizedMixLogistic(flax.linen.Module):
    ...
    def log_prob(self, x, dist_params):
        ...
        log_probs = distribution_utils_jax.discretized_mix_logistic(
            x, dist_params, gridsize)
        return log_probs

    def sample(self, key, params):
        sample_x = distribution_utils_jax.sample_from_discretized_mix_logistic(
...
class DiscretizedMixLogisticRGB(flax.linen.Module):
    ...
    def log_prob(self, x, params):
        ...
        log_probs = distribution_utils_jax.discretized_mix_logistic_rgb(
            x, params, gridsize)
        return log_probs

    def sample(self, key, params):
        sample_x = distribution_utils_jax.sample_from_discretized_mix_logistic_rgb(
            key, params, self.n_mixtures)  # range [-1., 1.]
    ...
    def encode(self, streams, x, params, selection):
        ...
        logits = distribution_utils_jax.logits_discretized_mix_logistic_rgb(
    ...
    def decode(self, streams, params, selection):
        ...
        # Decode Red.
        logits_r = distribution_utils_jax.logits_discretized_mix_logistic_rgb(
            jnp.asarray(x_decoded), params, self.n_classes)[Ellipsis, 0, :]
        probs_r = distribution_utils_jax.get_safe_probs(logits_r)
        ...
        # Decode Green.
        logits_g = distribution_utils_jax.logits_discretized_mix_logistic_rgb(
            jnp.asarray(x_decoded), params, self.n_classes)[Ellipsis, 1, :]
        probs_g = distribution_utils_jax.get_safe_probs(logits_g)
        ...
        # Decode Blue.
        logits_b = distribution_utils_jax.logits_discretized_mix_logistic_rgb(
            jnp.asarray(x_decoded), params, self.n_classes)[Ellipsis, 2, :]
        probs_b = distribution_utils_jax.get_safe_probs(logits_b)
