...
def sample_categorical_and_log_prob(key, logits):
    ...
    category_onehot = onehot(category, num_classes=logits.shape[-1])
...
def unpack_params(params, nr_mix, x_shape):
    # 10 = [1 (mixtures) + 3 (means) + 3 (log_scales) + 3 (coeffs)].
    pi_logits = params[:, :, :, :nr_mix]  # mixture coefficients.
    remaining_params = params[:, :, :, nr_mix:].reshape(*x_shape, nr_mix * 3)
    means = remaining_params[:, :, :, :, :nr_mix]
    ...
    # For Green (x1) and Blue (variable x2).
    # There are 3 coeff channels, one for (x1 | x0) and two for (x2 | x0, x1)
    coeffs = remaining_params[:, :, :, :, 2 * nr_mix:]
    return pi_logits, means, pre_act_scale, coeffs


def condition_rgb_means(x, means, coeffs, nr_mix):
    """
    computes means given rgb pixel values of x
    """
    batchsize, height, width = x.shape[0:3]
    coeffs = jax.nn.tanh(coeffs)
    m1 = means[:, :, :, 0:1, :]
    m2 = means[:, :, :, 1, :] + coeffs[:, :, :, 0, :] * x[:, :, :, 0, :]
    m2 = m2.reshape(batchsize, height, width, 1, nr_mix)
    m3 = means[:, :, :, 2, :] + coeffs[:, :, :, 1, :] * x[:, :, :, 0, :] + coeffs[:, :, :, 2, :] * x[:, :, :, 1, :]
    m3 = m3.reshape(batchsize, height, width, 1, nr_mix)
    return jnp.concatenate([m1, m2, m3], axis=3)


def discretize(x, means, scale, gridsize):
    centered_x = x - means
    inv_stdv = jax.nn.softplus(scale)
...
def redistribute_tails(x, log_cdf_plus, log_one_minus_cdf_min, cdf_delta):
    ...
    # Here the tails of the distribution are assigned, if applicable.
    log_probs = is_last_bin * log_one_minus_cdf_min + (1. - is_last_bin) * log_cdf_delta
    log_probs = is_first_bin * log_cdf_plus + (1. - is_first_bin) * log_probs
...
def discretized_mix_logistic_rgb(x, params, gridsize):
    ...
    nr_mix = params.shape[3] // 10
    pi_logits, means, pre_act_scale, coeffs = unpack_params(params, nr_mix, x_shape=x.shape)

    x = x.reshape(*x.shape, 1)
    x = x.repeat(nr_mix, axis=-1)

    means = condition_rgb_means(x, means, coeffs, nr_mix)

    log_cdf_plus, log_one_minus_cdf_min, cdf_delta = discretize(x, means, pre_act_scale, gridsize)
...
def select_logistic(selection, means, log_scales, coeffs):
    ...
    means = jnp.sum(means * selection, axis=4)
    pre_act_scales = jnp.sum(log_scales * selection, axis=4)

    coeffs = jnp.sum(coeffs * selection, axis=4)
    return means, pre_act_scales, coeffs


def standard_logistic(u, means, pre_act_scales):
...
def sample_from_discretized_mix_logistic_rgb(seed, params, nr_mix):
    ...
    # unpack parameters
    pi_logits, means, pre_act_scales, coeffs = unpack_params(params, nr_mix, xshape)
    coeffs = jax.nn.tanh(coeffs)
...
def sample_from_discretized_mix_logistic(seed, params, nr_mix):
    params = jnp.transpose(params, 0, 2, 3, 1)
    rng = jax.random.PRNGKey(seed)
